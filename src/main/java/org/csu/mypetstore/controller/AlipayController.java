...
@SessionAttributes(value = {"cart","order","isLogin","myAccount","orderList","msg","orderOfUpdate","trade_no","out_trade_no"})//model的attribution通过该注释将对象放在了session作用域中，以后通过model.getAttribute可以取出对象
public class AlipayController {
    ...
    public String returnUrl(HttpServletRequest request, HttpServletResponse response,Model model) throws UnsupportedEncodingException, AlipayApiException {
        ...
        if(verifyResult){
            ...
            //修改订单的状态
            orderService.updateOrderState(1,Integer.parseInt(out_trade_no));
            return "pay/paysuccess";
        }else{
            return "pay/payfailure";
        }
    }
